...
public class ServiceClass {
	...
	public ServiceClass() {
		// get the files to red and write
	...
	private IUser logIn() {
		while(true) {
			try {
				...
				System.out.println("User didnt signed-up before !!");
		...
		while(true) {
			try {
				...
				while(true) {
					...
					System.out.println("or add new one (a) previous page(p): ");
					String selection = input.nextLine();	
		...
		while(true) {
			try {
				...
				if(selection.equalsIgnoreCase("d")) {
					if(complaint.getStatus() != Status.FIXED && changeStatusOfComplaint(complaint, Status.DELETED)) {
	...
	private boolean changeStatusOfComplaint(IComplaint complaint,Status newStatus) {
		...
		lines.add(String.format("%d;%d;%d;%d;%s;%s;%s",complaint.getComplaintId(),complaint.getCustomerId(),complaint.getProducerId(),complaint.getProductId(),complaint.getComplaintTitle(),complaint.getComplaint(),complaint.getStatus().toString()));
		if(complaintFile.writeToFile(buildStringToWrite(lines))) {
